  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      try {
        ...
        // Create a set of unique colors to check if quantization is needed
        const uniqueColors = new Set(pixelArray.map(p => JSON.stringify(p)));

        if (uniqueColors.size <= 1) {
          // If only one color, just return that color
          const singleColor = pixelArray[0];
          return resolve([rgbToHex(singleColor[0], singleColor[1], singleColor[2])]);
        }

        // Ensure colorCount is not greater than the number of unique colors
        const finalColorCount = Math.min(colorCount, uniqueColors.size);
        
        // The quantize library needs at least 2 colors to work with.
        if (finalColorCount < 2) {
             const singleColor = pixelArray[0];
             return resolve([rgbToHex(singleColor[0], singleColor[1], singleColor[2])]);
        }

        const colorMap = quantize(pixelArray, finalColorCount);
        if (!colorMap) {
            return reject(new Error("Color quantization failed to produce a palette."));
        }
    ...
    img.onerror = (err) => {
      ...
      reject(new Error("Could not load the image. It might be an invalid file or a network issue."));
    };
